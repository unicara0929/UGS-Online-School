...
export function ContractCsvUpload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/contracts/upload/preview', {
        method: 'POST',
        body: formData,
      })
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await fetch('/api/admin/contracts/upload/confirm', {
        method: 'POST',
  ...
  const downloadSample = () => {
    const csv = `userId,contractNumber,productName,amount,signedAt,status
user123,CONTRACT-2025-001,がん保険プラン,50000,2025-01-15,ACTIVE
user456,CONTRACT-2025-002,医療保険スタンダード,30000,2025-01-20,ACTIVE`

    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'contract_sample.csv'
    link.click()
  }
        ...
        <CardContent className="space-y-4">
          <div className="text-sm space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-slate-600">
              ...
              <li><code>contractNumber</code> - 契約番号（ユニーク）</li>
              <li><code>productName</code> - 商品名</li>
              <li><code>amount</code> - 保険料</li>
              <li><code>signedAt</code> - 契約日（YYYY-MM-DD形式、例: 2025-01-15）</li>
              <li><code>status</code> - ステータス（ACTIVE/CANCELLED/EXPIRED）</li>
            </ul>
          </div>
