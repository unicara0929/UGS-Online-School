...
export async function POST(request: NextRequest) {
  try {
    ...
    // ファイル名を生成（タイムスタンプ_元のファイル名）
    const timestamp = Date.now()
    const sanitizedFileName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_')
    const fileName = `event-thumbnails/${timestamp}_${sanitizedFileName}`

    // Supabase Storageにアップロード（プロフィール画像バケットを使用）
    const bucketName = 'profile-picture'

    console.log('Uploading event thumbnail to bucket:', bucketName)
    ...
    console.error('Error uploading event thumbnail to Supabase Storage:', error)
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Error deleting event thumbnail from Supabase Storage:', error)
    return NextResponse.json(
