...
import { Loader2, Plus, Calendar, Archive, FileText, Users, Edit, Trash2, ChevronRight, Video, MapPin } from 'lucide-react'
...
function AdminEventsPageContent() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/admin/events?category=REGULAR', {
        credentials: 'include',
      })
  ...
  const handleCreateArchiveEvent = async (data: {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || '過去イベントの作成に失敗しました')
      }

      setShowArchiveForm(false)
      await fetchEvents()
    } catch (err) {
      console.error('Failed to create archive event:', err)
      alert(err instanceof Error ? err.message : '過去イベントの作成に失敗しました')
    } finally {
      setIsSubmitting(false)
    }
  }
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-600">{isAdmin ? 'イベントの作成・編集・削除' : 'イベント参加者の確認'}</p>
