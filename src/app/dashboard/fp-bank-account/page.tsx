...
import { ArrowLeft, Building2, Eye, EyeOff, CreditCard, AlertCircle, CheckCircle } from "lucide-react"
...
function FPBankAccountPage() {
  ...
  // アクセス制御: FPロール OR fpPromotionApproved
  useEffect(() => {
    const checkAccess = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // FP昇格承認済み OR needsOnboarding（=FPロール or 承認済みMEMBER）
          if (data.fpPromotionApproved || !data.needsOnboarding) {
            setHasAccess(true)
          } else {
      ...
      // 口座情報を取得
      try {
        const response = await authenticatedFetch('/api/user/fp-bank-account')
        if (response.ok) {
          const data = await response.json()
        ...
        <main className="px-4 sm:px-6 lg:px-8 py-8">
          <div className="space-y-6 max-w-4xl mx-auto">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-slate-900">報酬受け取り口座の登録</h1>
                <p className="text-slate-600 mt-1">FPエイドとして活動するための振込先口座を登録してください</p>
              </div>
            </div>
            ...
            {promoted && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-6 flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <p className="text-lg font-bold text-green-800">オンボーディング完了！FPエイドに昇格しました</p>
                  <p className="text-sm text-green-700 mt-1">まもなくダッシュボードに移動します...</p>
                </div>
              </div>
            )}
...
export default function FPBankAccountPageComponent() {
  return (
    <ProtectedRoute>
      <FPBankAccountPage />
    </ProtectedRoute>
