...
12. [FPエイド向け動画ガイダンス機能](#fpエイド向け動画ガイダンス機能)
...
### 概要
FPエイドに昇格したユーザーに対して、FPエイドとしての活動を開始する前に必須で視聴する動画ガイダンス機能です。動画の90%以上を視聴することで完了となります。
...
#### 12.2 アクセス制限
動画ガイダンス未完了のFPエイドは、以下の機能にアクセスできません：
...
1. **FPエイド昇格後の自動リダイレクト**
   - FPエイドに昇格すると、`fpOnboardingCompleted`が`false`に設定される
   - 次回ログイン時、またはダッシュボードにアクセスすると、自動的に動画ガイダンスページにリダイレクトされる
...
3. **完了後の動作**
   - 動画ガイダンス完了後、通常のFPエイド機能にアクセス可能になる
   - 次回以降は動画ガイダンスページにリダイレクトされない

### APIエンドポイント

- `GET /api/user/fp-onboarding-status` - FPエイド向け動画ガイダンスの完了状況を取得
- `POST /api/user/fp-onboarding/complete` - 動画ガイダンスの視聴完了を記録

### セキュリティ対策

- **フロントエンド**: `FPOnboardingGuard`コンポーネントで画面遷移を制御
- **バックエンド**: FPエイド機能のAPIでオンボーディング完了状況をチェック
- **抜け道防止**: クライアント側だけでなく、API側でもチェックを実装

### 注意事項

- 動画URLは環境変数`NEXT_PUBLIC_FP_ONBOARDING_VIMEO_ID`で設定します
- 既存のFPエイドユーザーは自動的に完了済みとして扱われます（デフォルト値: `true`）
- 新規にFPエイドに昇格したユーザーのみ、動画ガイダンスが必須となります
