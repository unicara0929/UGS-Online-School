...
- **申請フロー**:
  1. 条件達成状況の確認
  2. 昇格申請の送信（ステータス: `PENDING`）
  3. 管理者による承認（ステータス: `PENDING` → `APPROVED`）
- **承認時の処理**:
  - ユーザーロールが`FP`に更新
  - SupabaseとPrismaの両方で更新
  - 昇格通知を送信
  - FPオンボーディング未完了フラグを設定
- **承認後の手続き**（すべて完了後に`COMPLETED`ステータスに遷移）:
  1. **業務委託契約書への同意** - 契約内容の確認と同意
  2. **身分証のアップロード** - 本人確認書類の提出
  3. **FPオンボーディング動画の視聴** - FPエイド業務の動画ガイダンス（90%以上視聴で完了）
- **完了条件**:
  - 上記3つの手続きがすべて完了した時点で、昇格申請ステータスが`COMPLETED`に更新される
  - 完了するまでFPエイド機能へのアクセスが制限される

#### 3.2 マネージャー昇格
- **昇格条件**:
  1. 報酬実績: 直近6ヶ月の平均報酬が70,000円以上
  2. UGS会員紹介: 6ヶ月間に8名以上
  3. FPエイド紹介: 6ヶ月間に4名以上
  4. 契約実績: 直近20件の保険契約達成
- **申請フロー**:
...
#### コード構造
- **モジュール化**: 機能ごとに適切にモジュール化し、再利用可能な関数・コンポーネントを作成
- **関心の分離**: ビジネスロジック、UI、データアクセスを適切に分離
- **DRY原則**: 重複コードを避け、共通ロジックをヘルパー関数・サービス関数に抽出

#### エラーハンドリング
- **統一されたエラーレスポンス**: すべてのAPIルートで一貫したエラーレスポンス形式を使用
- **適切なHTTPステータスコード**: エラーの種類に応じて適切なステータスコードを返す
- **エラーログ**: エラー発生時に適切なログを記録し、デバッグを容易にする

#### 型安全性
- **TypeScriptの活用**: すべてのコードでTypeScriptを使用し、型安全性を確保
- **インターフェース定義**: データ構造を明確に定義し、型エラーを防止

#### パフォーマンス
- **データベース接続プール**: Supabase Transaction Poolerを使用し、接続プールを最適化
- **クエリ最適化**: 不要なデータ取得を避け、必要なデータのみを取得
- **キャッシュ**: 適切な場所でキャッシュを活用し、パフォーマンスを向上
...
- `idDocumentUploadedAt`: 身分証アップロード日時
- `contractAgreed`: 業務委託契約書への同意
- `contractAgreedAt`: 契約書同意日時
- `promotionEmailSent`: 昇格通知メール送信フラグ
- `promotionEmailSentAt`: 昇格通知メール送信日時
- `appliedAt`: 申請日時
- `approvedAt`: 承認日時
- `completedAt`: 完了日時（3つの手続き完了時）
